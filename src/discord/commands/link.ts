...
export async function executeLink(
  ...
  if (status.linked) {
    ...
    // M3: also reconcile rule-based roles so a member who linked elsewhere
    // lands their roles the moment they hit /link in a new server.
    ...
    if (interaction.guildId) {
      ...
      if (summary.notVerified === 'no_rules') {
        verifyNote = 'No verification rules in this server.';
      } else if (summary.notVerified === 'not_linked') {
        // Impossible here (status.linked), but never crash on it.
        verifyNote = null;
      } else if (summary.granted.length > 0 || summary.revoked.length > 0) {
        verifyNote = `${summary.granted.length} role${summary.granted.length === 1 ? '' : 's'} granted, ${summary.revoked.length} removed.`;
      }
    }
